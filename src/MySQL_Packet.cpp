/*
  ...
  Version 1.1.1ct Updated by Dr. Franco Ceotto, April 2016.
*/
#include <Arduino.h>
#include <MySQL_Packet.h>
#include <MySQL_Encrypt_Sha1.h>

#define MYSQL_DATA_TIMEOUT  5000   // client wait in milliseconds
#define MYSQL_WAIT_INTERVAL 1      // network client wait interval
...
/*
  ...
  Recoded for performance: ceox 2016.04.28
*/
int MySQL_Packet::wait_for_client() {
  unsigned long st = millis ();

  while ((millis()-st) < MYSQL_MAX_TIMEOUT) {
    if (client->available() < MYSQL_MIN_BYTES) {
...
/*
  ...
  data is available for more than MYSQL_DATA_TIMEOUT milliseconds in MYSQL_WAIT_INTERVAL
...
boolean MySQL_Packet::wait_for_data() {
  ...
  while (milliseconds < MYSQL_DATA_TIMEOUT) {
    if (client->available()) {
      return true;
    }
